...
SHEETS = [
    ...
    ("峡谷と火山の地形タイルシート.png", "canyon", "峡谷"),
    ("ファンタジーフォレストと洞窟の地面.png", "forest", "森床"),
    ("湖畔と市場の地面タイルシート.png", "lake", "湖畔"),
    ("ドリームタワーの床タイルシート.png", "tower", "塔床"),
    ("ファンタジー背景用ドット絵タイルシート.png", "bg", "背景"),
    ("魔法の階段と橋のピクセルタイル.png", "bridge", "階段橋"),
    # ---- オブジェクトタイル(上に置く用の建物・自然物) ----
    # ラベル名は実際の中身に合わせて命名(ファイル名の建前とは違う場合あり)
    ("ファンタジー村の16軒の家.png", "houseC", "家"),                     # 普通のコテージ集
    ("夢物語の16の家マップセット.png", "houseA", "夢家"),                  # ツリーハウス・キノコ家など特殊家
    ("夢物語の町の建物スプライトシート.png", "houseB", "施設"),            # 教会・店・城など大型建物
    ("16種類の樹木ピクセルアートシート.png", "trees", "大樹"),             # 大きな樹木16種
    ("夢幻の森のゲーム用スプライトシート.png", "wilds", "森木"),           # 森の木・茂み・キノコ
    ("ファンタジー風自然オブジェクトシート.png", "small", "小植物"),        # 小さな草・花・切株
    ("夢幻世界の特別地物スプライトシート.png", "facility", "自然物"),      # 岩・水辺物・水晶・門
    ("ポケモン風ドット絵地形パーツ.png", "terrain", "地形"),               # 岩・崖・階段・柵
    ("ポケモン風RPGオブジェクトシート.png", "plants", "生活物"),           # 井戸・柵・カート・看板
    ("夢の街の装飾オブジェクトシート.png", "decor", "街小物"),             # 看板・宝箱・彫像
]
